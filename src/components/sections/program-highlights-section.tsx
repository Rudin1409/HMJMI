...
export function ProgramHighlightsSection() {
  return (
    <section id="program" className="w-full py-20 lg:py-32 relative overflow-hidden">
      <BackgroundBlobs />
        ...
        <div className="text-center mb-16">
          ...
          <ScrollAnimation delay={1}>
            <h2 className="text-3xl md:text-5xl font-extrabold text-slate-900 dark:text-white mb-6 tracking-tight">
              Dari <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-700 via-pink-600 to-blue-600">Teori</span> ke <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-pink-600 to-purple-700">Aksi</span>
            </h2>
          </ScrollAnimation>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {highlightedPrograms.map((program, index) => (
            <ScrollAnimation key={index} delay={index + 1}>
              <SpotlightCard className="relative overflow-hidden group border-white/40 dark:border-white/10 bg-white/40 dark:bg-black/50 hover:bg-white/50 dark:hover:bg-white/10 transition-all duration-500 ease-out hover:shadow-2xl hover:shadow-pink-500/10 hover:-translate-y-2 hover:border-primary/30 h-full" spotlightColor="rgba(236, 72, 153, 0.2)">
                <div className="absolute top-0 right-0 w-40 h-40 bg-pink-500/10 blur-[40px] rounded-full -translate-y-1/2 translate-x-1/2 group-hover:bg-pink-500/20 transition-all duration-700" />
                ...
                <CardContent className="p-0 relative z-10 flex flex-col h-full">
                  <div className="relative aspect-video overflow-hidden rounded-t-[calc(var(--radius)-1px)]">
                    ...
                    <div className="absolute bottom-4 right-4 z-20 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 delay-100">
                      <div className="bg-white/95 dark:bg-black/85 p-3 rounded-full text-primary shadow-lg hover:scale-110 transition-transform cursor-pointer">
                  ...
                  <div className="p-8 flex flex-col gap-4 flex-grow">
                    <div className="flex items-start justify-between">
                      <Badge variant="secondary" className="bg-white/60 dark:bg-white/15 text-slate-900 dark:text-white border-white/20 group-hover:bg-primary/20 transition-colors">
                        {program.category}
                      </Badge>
                    </div>
