        ...
        <div className="text-center mb-16">
          ...
          <ScrollAnimation delay={1}>
            <h2 className="text-3xl md:text-5xl font-extrabold text-slate-900 dark:text-white mb-6 tracking-tight">
              Pertanyaan yang <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-700 via-pink-600 to-blue-600">Sering Diajukan</span>
            </h2>
          </ScrollAnimation>
        </div>
        ...
        <Accordion type="single" collapsible className="w-full space-y-4">
          {faqItems.map((item, index) => (
            <ScrollAnimation key={index} delay={index + 2}>
              <AccordionItem
                value={`item-${index}`}
                className="group border border-white/40 dark:border-white/10 bg-white/40 dark:bg-black/50 transition-all duration-300 ease-out rounded-2xl px-6 data-[state=open]:border-primary/50 data-[state=open]:bg-white/55 dark:data-[state=open]:bg-white/10 data-[state=open]:shadow-xl data-[state=open]:shadow-primary/10 hover:border-primary/30"
              >
